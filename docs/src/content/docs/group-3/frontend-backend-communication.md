...
## Purpose
The purpose of the frontend-backend communication documentation is to showcase how we handle the communication between the frontend and the backend. This communication is important to get right, as we otherwise cannot communicate or provide the user with a response to their request. Additionally, we do not want the frontend to call the backend for results before they are ready. This is done to avoid falsly displaying error message to the user when data simply is not ready yet, but will be ready soon.
...
The function is an asynchronous flow, awaiting a successful post to the backend. The mehtod is POST, and if the response returned by the backend (for the initial request) is not OK, an error is logged. The function takes two parameters: the url as a string and the data which is send to the backend as the data structure showcased above.
```typescript
...
This is the response data structure the backend sends to the frontend. It contains relevant information such as the distance in kilometers (as a `number`), travel time in minuts (as a `number`), and the route itself (as an `array` of `number`).
```typescript
{
    correlationId: string;
    origin: string;
...
When displaying the result to the user, we make sure to change the boolean flags `mapRouteLoading` and `displayLoading` to false, making sure that the results is actually visable in the frontend.
```typescript
const responseData = await response.json();
