...
│                                                                             │
│  ┌─────────────────────────────────┐    ┌─────────────────────────────────┐ │
│  │        C# Backend (.NET 9)      │    │      Python Backend (FastAPI)   │ │
│  │                                 │    │                                 │ │
│  │  ┌───────────────────────────┐  │    │  ┌───────────────────────────┐  │ │
│  │  │   TrainingController      │  │    │  │   PythonController        │  │ │
│  │  │   - /Training/status      │──┼────┼──│  - /Python/generate-routes│  │ │
│  │  │   - /Training/start-train │  │    │  │  - /Python/vectors        │  │ │
│  │  └───────────────────────────┘  │    │  │  - /Python/train-lstm     │  │ │
│  │              │                  │    │  │  - /Python/predict-time   │  │ │
│  │              ▼                  │    │  └───────────────────────────┘  │ │
│  │  ┌───────────────────────────┐  │    │              │                  │ │
...
├── training-service.csproj          # Root project file (references C#/)
├── training-service.sln             # Solution file
│
├── C#/                              # C# Backend
│   ├── Program.cs                   # Application entry point
│   ├── training-service.csproj      # C# project configuration
│   ├── CustomConsoleFormatter.cs    # Custom logging formatter
│   │
│   ├── Configuration/
│   │   └── PythonBackendSetting.cs  # Python backend URL/endpoint config
│   │
│   ├── Controller/
│   │   └── TrainingController.cs    # REST API endpoints
│   │
│   ├── Domain/
│   │   ├── Status.cs                # Thread-safe status tracker
│   │   ├── TrainingRequest.cs       # Training request DTO
│   │   └── TrainingSetModel.cs      # Training data models
│   │
│   ├── Infrastructure/
│   │   ├── ApiKeyAuthFilter.cs      # API key authentication
│   │   ├── ITrainingQueue.cs        # Queue interface
│   │   ├── TrainingQueue.cs         # In-memory job queue
│   │   └── TrainingWorker.cs        # Background job processor
│   │
│   └── Service/
│       └── Service.cs               # Core orchestration logic
│
├── Python/                          # Python Backend
...
│   │
│   ├── Controller/
│   │   └── PythonController.py      # FastAPI endpoints
│   │
│   └── Service/
│       ├── Data/
│       │   ├── TrainingSet.json     # Generated training data (runtime)
│       │   ├── LookupTableData/
│       │   │   ├── RoadNetwork.json     # Road graph structure
│       │   │   ├── embeddings.npy       # Edge vector embeddings
│       │   │   ├── traversals.csv       # Historical traversal times
│       │   │   └── lookupManager.py     # Memory-mapped data access
│       │   └── TrainedModels/
│       │       └── {ModelName}/         # Saved model checkpoints
│       │
│       ├── DatasetCreation/
│       │   ├── dataCreation.py          # Route generation
│       │   ├── timeCreation.py          # Traversal time calculation
│       │   ├── getEdgeToVectors.py      # Edge-to-vector conversion
│       │   └── embeddings_cache.py      # Embeddings caching
│       │
│       ├── VectorEmbedding/
│       │   └── Edge2Vec.py              # Node2Vec edge embedding generator
│       │
│       └── TrainingModels/
│           ├── TrainingModel.py         # LSTM model definition
│           ├── LSTMTraining.py          # Training loop
│           └── predictTime.py           # Inference utilities
│
└── Helpers/
